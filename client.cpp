...
// file request thread
void file_request_function(int m, bool file, string filename, FIFORequestChannel* chan, BoundedBuffer* request) {
    if (file) {
        // open file
        FILE *pFile;
...
void worker_thread_function(BoundedBuffer* request, BoundedBuffer* response, FIFORequestChannel* chan, int bufcap, string filename, bool file) {
...
int main(int argc, char *argv[])
{
	...
	int m = 2048; 	// default capacity of the message buffer
    int e = 1;              // default ecg value for requesting data points
    int h = 100;            // number of hist threads
    string filename = "100MB";
    bool file = true;
    srand(time_t(NULL));

    // get arguments
    while ((opt = getopt(argc, argv, "n:p:w:b:m:e:h:f:")) != -1) {
        switch (opt) {
            case 'n':
                n = atoi(optarg);
                break;
    ...
    if (file) {
        ...
        chan->cwrite(&channel, sizeof(MESSAGE_TYPE));
        char channelName[30];
        int nbytesChan = chan->cread(&channelName, 30);
